...
n = (
            ...
            MenuItem("setting",action=setting),
            MenuItem("quit",action=quiti),
        )
icn = Image.open(BytesIO(dedata))
ico = tk.PhotoImage(data=dedata)
...
listbox.bind("<<ListboxSelect>>", display_set)
...
tk.Button(button_frame, text="删除",
          command=lambda: delete(listbox.curselection())).pack(pady=5)
tk.Button(button_frame, text="复制",
          command=lambda: get(listbox.curselection())).pack(pady=5)
bu = tk.Button(button_frame, text="启动/展示", command=start)
